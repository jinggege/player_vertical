package com.laifeng.controls
{
	...
	public class LFExtenrnalInterface
	{
		...
		/**player 调用 JS 接口：    显示推荐 */
		public const  INTERFACE_SHOW_RECOMMEND:String ="_flash_show_recommend";
		/**player 调用 JS 接口：    换房间*/
		public const INTERFACE_CHANGE_ROOM:String = "_flash_change_room";
		
		private var _playerStatus:String = 'stop'; //playing / stop
		...
		public function switchRoom():void{
			ExternalInterface.call(INTERFACE_CHANGE_ROOM);
		}
		
		/**
		 * 是否显示推荐
		 * @param status:0/1  0:隐藏  1:显示
		 */
		public function showRecommend(status:int):void{
			if(status==1){
				if(this._playerStatus !='playing'){
					this.playerCallJs(INTERFACE_SHOW_RECOMMEND,1);
				}
			}else{
				this.playerCallJs(INTERFACE_SHOW_RECOMMEND,0);
			}
		}
		
		
		/**
		 * @param interfaceName 调用JS接口名称
		 * @param data 调用JS接口参数
		 */
		public function playerCallJs(interfaceName:String,data:Object):void{
			try{
				ExternalInterface.call(interfaceName,data);
			}catch(e:Error){}
		}
		
		
		/**
		 * @param interfaceName 调用player接口名称
		 * @param data 调用player接口参数
		 */
		public function jsCallPlayer(interfaceName:String,data:Object):void{
			//todo  所有JS 调用Player  接口都由此调用
		}
